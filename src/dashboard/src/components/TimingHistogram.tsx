import { useQuery } from "@tanstack/react-query";
import axios from "axios";
import {
  BarChart,
  Bar,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  ResponsiveContainer,
} from "recharts";

const API_BASE = import.meta.env.VITE_API_URL || "http://localhost:3000";

export default function TimingHistogram() {
  const { data, isLoading } = useQuery({
    queryKey: ["timing-distribution"],
    queryFn: async () => {
      const res = await axios.get(`${API_BASE}/api/timing-distribution`);
      return res.data;
    },
  });

  if (isLoading) {
    return (
      <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700 h-80 flex items-center justify-center">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-500"></div>
      </div>
    );
  }

  // Transform bucket data for chart
  const chartData =
    data?.buckets?.slice(0, 24).map((item: any) => ({
      hours: item.hours,
      count: item.count,
    })) || [];

  const hasData = chartData.length > 0;

  const formatTime = (hours: number) => {
    if (hours < 24) return `${hours}h`;
    const days = Math.floor(hours / 24);
    return `${days}d`;
  };

  return (
    <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-white">
          Timing Distribution
        </h3>
        <p className="text-sm text-slate-400">
          Time between deposit and withdrawal
        </p>
      </div>

      {!hasData ? (
        <div className="h-64 flex flex-col items-center justify-center text-center">
          <div className="text-slate-400 mb-2">
            No withdrawal data available
          </div>
          <div className="text-sm text-slate-500">
            This protocol has deposits but no withdrawals yet.
            <br />
            Timing analysis requires withdrawal activity.
          </div>
        </div>
      ) : (
        <>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={chartData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
              <XAxis
                dataKey="hours"
                stroke="#94a3b8"
                tickFormatter={formatTime}
                label={{
                  value: "Time (hours)",
                  position: "insideBottom",
                  offset: -5,
                  fill: "#94a3b8",
                }}
              />
              <YAxis
                stroke="#94a3b8"
                label={{
                  value: "Count",
                  angle: -90,
                  position: "insideLeft",
                  fill: "#94a3b8",
                }}
              />
              <Tooltip
                contentStyle={{
                  backgroundColor: "#1e293b",
                  border: "1px solid #475569",
                  borderRadius: "0.5rem",
                  color: "#e2e8f0",
                }}
                labelFormatter={formatTime}
              />
              <Bar dataKey="count" fill="#c084fc" />
            </BarChart>
          </ResponsiveContainer>

          <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
            <div>
              <div className="text-slate-400">Median</div>
              <div className="text-white font-semibold">
                {formatTime(Math.floor((data?.median || 0) / (3600 * 1000)))}
              </div>
            </div>
            <div>
              <div className="text-slate-400">Entropy</div>
              <div className="text-white font-semibold">
                {data?.entropy?.toFixed(2) || 0} bits
              </div>
            </div>
            <div>
              <div className="text-slate-400">Pattern</div>
              <div className="text-white font-semibold">
                {data?.entropy > 4 ? "Random" : "Predictable"}
              </div>
            </div>
          </div>
        </>
      )}
    </div>
  );
}
